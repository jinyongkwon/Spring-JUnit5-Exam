...
public class BookApiControllerWebMvcTest {
    ...
    @Test
    public void bookFindOne_테스트() throws Exception {
        // given
        Long id = 1L;

        // stub
        Mockito.when(bookService.책한건가져오기(id)).thenReturn(new BookRespDto(1L, "제목1", "메타코딩"));

        // when
        ResultActions resultActions = mockMvc.perform(
                MockMvcRequestBuilders.get("/api/book/" + id));
